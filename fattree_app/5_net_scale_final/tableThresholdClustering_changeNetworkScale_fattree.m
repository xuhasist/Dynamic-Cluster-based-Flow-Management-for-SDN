function tableThresholdClustering_changeNetworkScale_fattree(roundNumber)
    t1 = datetime('now');
    ...
    % fat-tree pods
    fattreeK_list = [4, 8, 16];

    x_axis = fattreeK_list;
    x_label = 'Number of Fat-tree Pods';
    ...
    for frequency = 1:roundNumber
        ...
        for networkScale = 1:length(fattreeK_list)
            k = fattreeK_list(networkScale);

            [swNum, srcNode, dstNode, srcInf, dstInf, g, hostNum, IP] = ...
                createFatTreeTopo_networkScale(k, hostAvg, hostSd);
            ...
            filename = ['memory/memory_', int2str(frequency), '_', int2str(k)];
            save(filename)
        end
