function [linkThputStruct, linkPreLower] = updateLinkStruct(finalPath, g, linkThputStruct, flowStartDatetime, flowEndDatetime, linkPreLower, flowEntry, flowRate)
    flowEndStrtime = datestr(flowEndDatetime, 'yyyy-mm-dd HH:MM:ss.FFF');
    
    % update link load along this path
    for j = 1:length(finalPath)-1
        ...
        [~, ind] = sortrows({linkThputStruct(edge).entry.startTime}');
