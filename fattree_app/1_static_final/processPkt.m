function [finalPath, swFlowEntryStruct, linkTable, finish] = ...
    ...
    if strcmp(srcEdgeSw, dstEdgeSw)
        ...
        if strcmp(dstEdgeSw, swInfTable{rows, {'DstNode'}}{1}) && round == 3 % last round
    ...
    if ~any(flow_compare) % miss
        % find a least congested shortest path
        ...
        % find switch output port of flow entry
        ...
        if ~isempty(sameFlowLoc)
            ...
            % calculate new timeout of flow entry 
        ...
        if isempty(sameFlowLoc) || (newFlowStartTime - oldFlowEndTime > seconds(60))
            ...
            % find switch input port of flow entry for next hop
...
function swFlowEntryStruct = installFlowRule(g, path, swInfTable, hostIpTable, swFlowEntryStruct, flowEntry, round, dstNodeName)
    for j = 2:length(path)-1    
        % find switch input/output port of flow entry
        ...
        % check if the same flow rules exist in the switch
...
function [path, linkTable] = firstFitFlowScheduling(g, linkTable, linkBwdUnit, linkPreLower, linkThputStruct, srcEdgeSw, dstEdgeSw, flowStartDatetime)
    ...
    % return all shortest pathes
    allShortestPath = dijkstra(g, srcNode, dstNode);
        
    bestPath.congest = -1;
    bestPath.path = '';
    
    % find the max throughput path
