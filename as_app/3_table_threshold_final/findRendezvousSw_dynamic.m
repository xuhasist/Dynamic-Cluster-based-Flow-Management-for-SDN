function middleSw = findRendezvousSw_dynamic(edgeSw, g, swDistanceVector, linkBwdUnit, linkTable, linkPreLower, linkThputStruct, flowStartDatetime, swWithTooManyFlowEntry)
    middleSw = -1;
    distance = 1;
    ...
    while middleSw == -1
        for k = 1:length(nodeNum)
            ...
            rows = ismember(tmp_list{k}, swWithTooManyFlowEntry);
            tmp_list{k}(rows) = [];
        end
