function [finalPath, swFlowEntryStruct, linkTable, finish] = ...
    ...
    flowStartDatetime, srcEdgeSw, dstEdgeSw, round, dstNodeName, flowSrcIp, flowDstIp, flowId)
    ...
    if strcmp(srcEdgeSw, dstEdgeSw)
        ...
        if strcmp(dstEdgeSw, swInfTable{rows, {'DstNode'}}{1}) && round == 3
            ...
            swFlowEntryStruct = installLastEdgeSwFlowRule(g, swInfTable, swFlowEntryStruct, flowEntry, dstEdgeSw, dstNodeName, flowId);
    ...
    flow_compare = arrayfun(@(x) startsWith(flowSrcIp, x.srcIp) & startsWith(flowDstIp, x.dstIp) & isequal(flowEntry.vlan, x.vlan) & isequal(flowEntry.input, x.input), swFlowEntryStruct(sw).entry);
    
    if ~any(flow_compare) % miss
        ...
        swFlowEntryStruct = installFlowRule(g, path, swInfTable, hostIpTable, swFlowEntryStruct, flowEntry, round, dstNodeName, flowId);
    else
        ...
        if isempty(sameFlowLoc) || (newFlowStartTime - oldFlowEndTime > seconds(60)) % miss
            [path, linkTable] = firstFitFlowScheduling(g, linkTable, linkBwdUnit, linkPreLower, linkThputStruct, srcEdgeSw, dstEdgeSw, flowStartDatetime);
            ...
            swFlowEntryStruct(sw).entry(sameFlowLoc).flowId = [swFlowEntryStruct(sw).entry(sameFlowLoc).flowId, flowId];
...
function swFlowEntryStruct = installFlowRule(g, path, swInfTable, hostIpTable, swFlowEntryStruct, flowEntry, round, dstNodeName, flowId)
    for j = 2:length(path)-1    
        ...
        flow_compare = arrayfun(@(x) isequal(flowEntry.srcIp, x.srcIp) & isequal(flowEntry.dstIp, x.dstIp) & isequal(flowEntry.vlan, x.vlan) & isequal(flowEntry.input, x.input) & isequal(flowEntry.output, x.output), swFlowEntryStruct(sw).entry);
...
function swFlowEntryStruct = installLastEdgeSwFlowRule(g, swInfTable, swFlowEntryStruct, flowEntry, dstEdgeSw, dstNodeName, flowId)
    sw = findnode(g, dstEdgeSw);
    flow_compare = arrayfun(@(x) isequal(flowEntry.srcIp, x.srcIp) & isequal(flowEntry.dstIp, x.dstIp) & isequal(flowEntry.vlan, x.vlan) & isequal(flowEntry.input, x.input), swFlowEntryStruct(sw).entry);
